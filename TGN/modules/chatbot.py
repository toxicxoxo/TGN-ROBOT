...
async def hmm(_, message):
    ...
    if status == "ON" or status == "on" or status == "On":
        ...
        if not lol:
            await lel.edit("HOMIESBot AI Already Activated In This Chat")
            return
        await lel.edit(f"HOMIESBot AI Actived by {message.from_user.mention()} for users in {message.chat.title}")

    elif status == "OFF" or status == "off" or status == "Off":
        lel = await edit_or_reply(message, "`Processing...`")
        Escobar = remove_chat(int(message.chat.id))
        if not Escobar:
            await lel.edit("HOMIESBot AI Was Not Activated In This Chat")
            return
        await lel.edit(f"HOMIESBot AI Deactivated by {message.from_user.mention()} for users in {message.chat.title}")
...
__help__ = """
   RealHOMIES-BOT AI 3.0 IS THE ONLY AI SYSTEM WHICH CAN DETECT & REPLY UPTO 200 LANGUAGES
